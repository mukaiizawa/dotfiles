...
-- SET EDITFILE EDIT コマンド で使用する一時ファイル名の設定 (エディタの指定は _EDITOR 事前定義変数)
-- SET EMBEDDED 異なるレポート(SQL)でも同一ページ含める／含めない
-- SET ERRORLOGGING Oracle 11g 実行中に発生した例外を指定したテーブルに出力する
-- SET ESCAPE エスケープ機能を有効／無効、または、別のエスケープ文字を設定
-- SET ESCCHAR Oracle 11g ファイル名を指定するときにエスケープして意味を持たせない文字を指定
-- SET EXITCOMMIT Oracle 11g Release2 EXIT(QUIT) 時のデフォルトの振る舞いで COMMIT するか ROLLBACK するかの設定
